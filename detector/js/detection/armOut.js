 ...
 */
 function moveArmOut(arm, mult, shoulderRotation) {
    ...
    if (arm[0].visibility >= 0.65 && arm[1].visibility >= 0.65 && arm[2]) { //當visibility >= 0.65 時在webcam的畫面上才會出現偵測點及線(有偵測到)
        ...
        let upperZo = (getRotation(arm[1], arm[0]) - shoulderRotation); //參考 openvtuber的寫法
        ...
        if(armAngle > 1.9 && arm[2].y > arm[1].y){ //手自然垂放在腿旁邊情況 讓上手臂可以安分並正確的放好
            // console.log(upperZ)
            if(mult == 1){
                upperZ = rad(-75);
            }
