...
function moveArmIn(arm, mult) {
    ...
    if (arm[0].visibility >= 0.65 && arm[1].visibility >= 0.65 && arm[2]) { //當visibility >= 0.65 時在webcam的畫面上才會出現偵測點及線(有偵測到)
        ...
        //手放在肚子前(我們的右手) (upperY乘0.2為了讓手臂敞開一點)
        if(mult == -1 && arm[0].x + 0.3 > arm[2].x && arm[1].y < arm[2].y){
            upperY *= 0.2 
        } //手放在肚子前(我們的左手) (upperY乘0.2為了讓手臂敞開一點) //230712
        if(mult == 1 && arm[0].x + 0.3 < arm[2].x && arm[1].y < arm[2].y){
            upperY *= 0.2
        } //手放在胸前(我們的右手) (upperY乘1.3為了讓手臂往內一點) 
        else if(mult == -1 && arm[0].x > arm[2].x && arm[1].y > arm[0].y && arm[2].y + 0.1 < arm[1].y){
            upperY *= 1.3 
        } //手放在胸前(我們的左手) (upperY乘1.3為了讓手臂往內一點) 
        else if(mult == 1 && arm[0].x < arm[2].x && arm[1].y > arm[0].y && arm[2].y + 0.1 < arm[1].y){
            upperY *= 1.3
        }

        
        //手臂不重疊在身體上(我們右手 : 手腕x > 肩膀x，我們左手 : 肩膀x > 手腕x) 且 手腕高於手肘 
        //讓我們做揮手動作時 vrm手掌可以成功朝前 手掌方向不會往身體方向偏
        if (arm[0].x > arm[2].x && mult == 1 && arm[1].y > arm[2].y){ //&& arm[0].y < arm[1].y
            upperY *= 0.2 * mult
            //230712
            upperZ *= 0.8
        }
        ...
        //左右手 高舉手的時候 下手臂讓他伸直 
        if(armAngle > 0.75  && arm[1].y < arm[0].y - 0.24){
            lowerZ = 0;
            lowerY = 0;
        }
