...
//自己先前設定的版面配置
const layoutsChoose = sessionStorage.getItem("layoutsChoose");  

//若先前有調整過版面配置 之後去了其他頁面 再回來detector 要把先前設置的版面配置調整回來 選項也要勾選相對應的版面配置選項
if (layoutsChoose != undefined) {
    layoutsChange(layoutsChoose);

    const id = "layouts" + layoutsChoose;
    document.getElementById("layouts2").checked = false;
